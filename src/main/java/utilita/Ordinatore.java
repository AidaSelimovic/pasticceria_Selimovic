...
import com.mycompany.pasticceria_selimovic.Pasticcino;
...
public class Ordinatore 
{
     ...
     * @param pos2 
     */
    public static void scambiaLibri(Pasticcino[] a, int pos1, int pos2)
    {
        Pasticcino s;
        s=a[pos1];
        a[pos1]=a[pos2];
     ...
     * @return  una copia dell'array ordinato
     */
    public static Pasticcino[] selectionSortCrescenteLibri (Pasticcino[] a)
    {
        Pasticcino v[]= new Pasticcino[a.length];
        int lunghezza=v.length;
        //creo una copia di a
        for(int i=0;i<v.length;i++)
        {
            v[i]= new Pasticcino(a[i]);      
        }
        ...
        for (int i=0;i<lunghezza-1;i++)
        {
            for(int j=i+1;j<lunghezza;j++)
            {
                if (v[j].getTipo().compareToIgnoreCase(v[i].getTipo())<0)
                    scambiaLibri(v, i, j);
            }
        }       
        return v;    
